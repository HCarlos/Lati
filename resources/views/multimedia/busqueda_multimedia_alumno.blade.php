    ...
    <div class="flex-center position-ref">
        <div class="row">
            <div class="content col-md-12">
                <img src="{{asset('assets/img/biblioteca_virtual_logo_1.png')}}" width="400" height="100" /><br/>
                <div class="content text-left">
                @if( count($items) <= 0)
                    <div class="alert alert-danger" role="alert">Resultado de la busqueda: <strong>{{count($items)}}</strong> elementos.</div>
                @else
                ...
                @foreach($items as $lib)
                    <div class="card">
                        <div class="card-header"><strong>{{$lib->titulo}}</strong></div>
                        <div class="card-body">
                            <table>
                                <tr>
                        ...
                        <div class="card-footer row-fluid">
                            @foreach($lib->imagenes as $item)
                                <a href="{{ asset('storage/'.$item->root.$item->filename)  }}" target="_blank" class="img-thumbnail pull-left">
                                    <img src="{{ asset('storage/'.$item->root.$item->filename)  }}" width="100" height="80" title="{{$item->title}}" class="img-responsive"/>
                                </a>
                            @endforeach
                        </div>
                    </div>
